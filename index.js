...
function syncJson(src, dst, props, callback, progress) {
  ...
  if (typeof src === 'string') srcObj = readJsonSync(src)
    ...
    let msg = 'Parameter "src" must be a string or an object, not ' + typeof src
    throw new TypeError(msg)
  }
  // dst param typecheck
  if (typeof dst === 'string') dst = [dst]
  else if (!Array.isArray(dst)) {
    let msg = 'Parameter "dst" must be a string or an array, not ' + typeof dst
    throw new TypeError(msg)
  }
  // props param typecheck
  if (typeof props === 'string') props = srcObj[props]
  if (!props) props = Object.keys(srcObj)
  // defaults
    ...
    let msg = 'Parameter "props" of unrecognized type ' + typeof props
    throw new TypeError(msg)
  }
  // callback param typecheck
  if (typeof callback !== 'function') {
    let msg = 'Parameter "callback" must be a function, not ' + typeof callback
    throw new TypeError(msg)
  }
